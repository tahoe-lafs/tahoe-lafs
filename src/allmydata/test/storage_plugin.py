...
class DummyStorage(object):
    ...
    def _section_name(self):
        return u"storage.plugins.{}".format(self.name)

    def get_storage_server(self, configuration, get_anonymous_storage_server):
        config = configuration.items(self._section_name)
        if u"invalid" in config:
    ...
    def get_storage_client(self, configuration, announcement, get_rref):
        return DummyStorageClient(
            get_rref,
            configuration.items(self._section_name),
            announcement,
        )
    ...
    def get_client_resource(self, configuration):
        ...
        """
        return Data(
            dumps(configuration.items(self._section_name)),
            b"text/json",
        )
