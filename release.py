...
CONTINUE_INSTRUCTION = (
    bcolors.BOLD
    + "Instruction : run {path}/venv/bin/python release.py --ignore-deps --tag {tag} --ticket {ticket} --sign {sign} --fin".format(
        path=RELEASE_FOLDER, tag=TAG, ticket=TICKET, sign="YOUR_SIGNING_KEY_HERE"
    )
    + bcolors.ENDC
)
...
def clean():
    """
    Remove all files and folders generated by release proccess
    """
...
def start_release():
    ...
    print(
        f"{bcolors.OKBLUE}Instruction: Make sure your github username, password (not required for ssh targets) and gpg passphrase are ready for the next step."
    )
    print(
